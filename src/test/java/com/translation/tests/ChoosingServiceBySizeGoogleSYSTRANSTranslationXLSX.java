 ...
 */
public class ChoosingServiceBySizeGoogleSYSTRANSTranslationXLSX {

    //source and translation files:
    //public static final File sourceFile= new File("D:\\xlsx\\SourceSampleForTesting.xlsx"); // read from
    static String sourceFile = "SourceSampleForTesting.xlsx"; // if the file is directly under the project
    //private static final File transFile= new File("D:\\xlsx\\Nov15_2021\\Trans6.xlsx"); // to write into. A blank file with the name "Trans6.xlsx" must exist in the folder or under the project.
        ...
        for (int i=10; i <=rowNums; i++) {
            ...
            for (int j = 0; j < colNums; j++) {
                ...
                if (i > 0 && j == 0) {
                    ...
                    if (cellRContent.contains("-") && cellRContent.length() == 1) {
                        cellW.setCellValue("-");
                    } // if "-" is the phone number, it should remain so.
                    ...
                    } else if (cellRContent.length() > 1 && !cellRContent.contains("-")) {

                        if (cellRContent.contains("E")) { //"1.5999018373E10"
                            Double db = cellR.getNumericCellValue(); // having this line just before the last if, is a nightmare!
                            cellW.setCellValue(db.longValue() + "");
                        } // with db.longValue()+"", cellW= 15999018373; if db.longValue() only, then cellW=1.5999018373E10
                        else {
                            cellW.setCellValue(cellRContent); // Ex: 18199570928
                        }
